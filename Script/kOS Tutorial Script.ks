...
function distanceToMunAtApoapsis {
  parameter mnv.
  local apoapsisTime is improveConverge(list(time:seconds + mnv:eta), {
    parameter data.
    local t is data[0].
    local result is -(positionAt(ship, t) - positionAt(Kerbin, t)):mag.
    print result.
    return result.
  }).
  return (positionAt(ship, apoapsisTime) - position(Mun, apoapsisTime)):mag. //returns distance from Mun to ship at ship's apoapsis.
}
